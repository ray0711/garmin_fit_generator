...
var CrcCalculator = class _CrcCalculator {
  #crc = 0;
  constructor() {
  }
  get crc() {
  ...
  #updateCRC(value) {
    let tmp = crcTable[this.#crc & 15];
    this.#crc = this.#crc >> 4 & 4095;
    this.#crc = this.#crc ^ tmp ^ crcTable[value & 15];
    tmp = crcTable[this.#crc & 15];
    this.#crc = this.#crc >> 4 & 4095;
    this.#crc = this.#crc ^ tmp ^ crcTable[value >> 4 & 15];
    return this.#crc;
  }
...
var Stream = class _Stream {
  ...
  readUInt8(opts) {
    return this.readValue(fit_default.BaseType.UINT8, 1, __spreadValues({ convertInvalidToNull: false }, opts));
  }
  readInt8(opts) {
    return this.readValue(fit_default.BaseType.SINT8, 1, __spreadValues({ convertInvalidToNull: false }, opts));
  }
  readUInt16(opts) {
    return this.readValue(fit_default.BaseType.UINT16, 2, __spreadValues({ convertInvalidToNull: false }, opts));
  }
  readInt16(opts) {
    return this.readValue(fit_default.BaseType.SINT16, 2, __spreadValues({ convertInvalidToNull: false }, opts));
  }
  readUInt32(opts) {
    return this.readValue(fit_default.BaseType.UINT32, 4, __spreadValues({ convertInvalidToNull: false }, opts));
  }
  readInt32(opts) {
    return this.readValue(fit_default.BaseType.SINT32, 4, __spreadValues({ convertInvalidToNull: false }, opts));
  }
  readUInt64(opts) {
    return this.readValue(fit_default.BaseType.UINT64, 8, __spreadValues({ convertInvalidToNull: false }, opts));
  }
  readInt64(opts) {
    return this.readValue(fit_default.BaseType.SINT64, 8, __spreadValues({ convertInvalidToNull: false }, opts));
  }
  readFloat32(opts) {
    return this.readValue(fit_default.BaseType.FLOAT32, 4, __spreadValues({ convertInvalidToNull: false }, opts));
  }
  readFloat64(opts) {
    return this.readValue(fit_default.BaseType.FLOAT64, 8, __spreadValues({ convertInvalidToNull: false }, opts));
  }
  readString(strlen) {
...
var Decoder = class _Decoder {
  ...
  #stream = null;
  #accumulator = new accumulator_default();
  #messages = {};
  #fieldsWithSubFields = [];
  #fieldsToExpand = [];
  ...
  static isFIT(stream) {
    try {
      ...
      const fileHeader = _Decoder.#readFileHeader(stream, { resetPosition: true });
   ...
   */
  isFIT() {
    return _Decoder.isFIT(this.#stream);
  }
  /**
  ...
  checkIntegrity() {
    try {
      ...
      const fileHeader = _Decoder.#readFileHeader(this.#stream, { resetPosition: true });
  ...
  read({
    ...
    this.#optMergeHeartRates = mergeHeartRates2;
    this.#optDecodeMemoGlobs = decodeMemoGlobs2;
    ...
    try {
      ...
      if (this.#optMergeHeartRates) {
        utils_hr_mesg_default.mergeHeartRates(this.#messages.hrMesgs, this.#messages.recordMesgs);
      }
      if (this.#optDecodeMemoGlobs) {
        utils_memo_glob_default.decodeMemoGlobs(this.#messages);
      }
    } catch (error) {
      errors.push(error);
    } finally {
  ...
  #decodeNextFile() {
    ...
    this.#stream.crcCalculator = new crc_calculator_default();
    const { headerSize, dataSize } = _Decoder.#readFileHeader(this.#stream, { decodeMode: this.#decodeMode });
  ...
  #decodeMessageDefinition() {
    ...
    this.#mesgDefinitionListener?.(__spreadValues({}, messageDefinition));
    ...
    this.#localMessageDefinitions[messageDefinition.localMesgNum] = __spreadValues(__spreadValues({}, messageDefinition), messageProfile);
  ...
  #decodeMessage() {
    ...
    if (mesgNum === profile_default.MesgNum.DEVELOPER_DATA_ID) {
      ...
      const key = Object.keys(this.#developerDataDefinitions).reduce((count, key2) => count + this.#developerDataDefinitions[key2].fields.length, 0);
    ...
    if (messageDefinition.name != null) {
      ...
      if (mesgNum === profile_default.MesgNum.FIELD_DESCRIPTION && this.#fieldDescriptionListener != null) {
        const developerDataIdMesg = this.#messages.developerDataIdMesgs?.find((developerDataIdMesg2) => {
          return developerDataIdMesg2.developerDataIndex === message.developerDataIndex;
        }) ?? {};
        this.#fieldDescriptionListener(message.key, __spreadValues({}, developerDataIdMesg), __spreadValues({}, message));
      }
    }
  }
  ...
  #readRawFieldValue(messageDefinition, fieldDefinition, field) {
    const rawFieldValue = this.#stream.readValue(
      ...
      fieldDefinition.size,
      {
        ...
        convertInvalidToNull: !field?.hasComponents
      }
    );
  ...
  #transformValue(messageDefinition, field, rawFieldValue) {
    ...
    } else if (fit_default.NumericFieldTypes.includes(field?.type ?? -1)) {
  ...
  #applyScaleAndOffset(messageDefinition, field, rawFieldValue) {
    ...
    if (fit_default.NumericFieldTypes.includes(field?.type ?? -1) === false) {
      return rawFieldValue;
    }
    ...
    const scale = Array.isArray(field?.scale ?? fit_default.FIELD_DEFAULT_SCALE) ? field?.scale[0] : field?.scale ?? fit_default.FIELD_DEFAULT_SCALE;
    const offset = Array.isArray(field?.offset ?? fit_default.FIELD_DEFAULT_OFFSET) ? field?.offset[0] : field?.offset ?? fit_default.FIELD_DEFAULT_OFFSET;
    try {
      if (Array.isArray(rawFieldValue)) {
  ...
  #convertTypeToString(messageDefinition, field, rawFieldValue) {
    ...
    if (fit_default.NumericFieldTypes.includes(field?.type ?? -1)) {
      return rawFieldValue;
    }
    try {
      const type = profile_default.types[field?.type ?? -1];
      if (Array.isArray(rawFieldValue)) {
        return rawFieldValue.map((value) => {
...
var OutputStream = class {
  ...
  constructor({
    ...
    this.#baseTypeDefinitions = {
      [fit_default.BaseType.ENUM]: { setValue: this.#dataView.setUint8.bind(this.#dataView), size: 1, mask: 255 },
      [fit_default.BaseType.UINT8]: { setValue: this.#dataView.setUint8.bind(this.#dataView), size: 1, mask: 255 },
      [fit_default.BaseType.UINT16]: { setValue: this.#dataView.setUint16.bind(this.#dataView), size: 2, mask: 65535 },
      [fit_default.BaseType.UINT32]: { setValue: this.#dataView.setUint32.bind(this.#dataView), size: 4, mask: 4294967295 },
      [fit_default.BaseType.UINT64]: { setValue: this.#dataView.setBigUint64.bind(this.#dataView), size: 8, mask: 0xFFFFFFFFFFFFFFFFn },
      [fit_default.BaseType.SINT8]: { setValue: this.#dataView.setInt8.bind(this.#dataView), size: 1, mask: 255 },
      [fit_default.BaseType.SINT16]: { setValue: this.#dataView.setInt16.bind(this.#dataView), size: 2, mask: 65535 },
      [fit_default.BaseType.SINT32]: { setValue: this.#dataView.setInt32.bind(this.#dataView), size: 4, mask: 4294967295 },
      [fit_default.BaseType.SINT64]: { setValue: this.#dataView.setBigInt64.bind(this.#dataView), size: 8, mask: 0xFFFFFFFFFFFFFFFFn },
      [fit_default.BaseType.FLOAT32]: { setValue: this.#dataView.setFloat32.bind(this.#dataView), size: 4 },
      [fit_default.BaseType.FLOAT64]: { setValue: this.#dataView.setFloat64.bind(this.#dataView), size: 8 },
      [fit_default.BaseType.UINT8Z]: { setValue: this.#dataView.setUint8.bind(this.#dataView), size: 1, mask: 255 },
      [fit_default.BaseType.UINT16Z]: { setValue: this.#dataView.setUint16.bind(this.#dataView), size: 2, mask: 65535 },
      [fit_default.BaseType.UINT32Z]: { setValue: this.#dataView.setUint32.bind(this.#dataView), size: 4, mask: 4294967295 },
      [fit_default.BaseType.UINT64Z]: { setValue: this.#dataView.setBigUint64.bind(this.#dataView), size: 8, mask: 0xFFFFFFFFFFFFFFFFn },
      [fit_default.BaseType.BYTE]: { setValue: this.#dataView.setUint8.bind(this.#dataView), size: 1, mask: 255 }
    };
  }
  get length() {
  ...
  #setValues(baseType, value) {
    ...
    values.forEach((value2) => {
      return this.#setValue(baseType, value2);
    });
  }
...
var Encoder = class {
  ...
  #updateFileHeader() {
    ...
    dataView.setUint32(4, this.#outputStream.length - HEADER_WITH_CRC_SIZE2, true);
  ...
  #writeFileCrc() {
    const crc = crc_calculator_default.calculateCRC(this.#outputStream.uint8Array, 0, this.#outputStream.length);
    this.#outputStream.writeUInt16(crc);
  }
  #transformValues(value, fieldDefinition) {
    const values = Array.isArray(value) ? value : [value];
    return values.map((value2) => {
      return this.#transformValue(value2, fieldDefinition);
    });
  }
  ...
  #createMesgDefinition = (mesgNum, mesg) => {
    const mesgDefinition = new mesg_definition_default(mesgNum, mesg, { fieldDescriptions: this.#fieldDescriptions });
  ...
  #lookupLocalMesgNum = (mesgDefinition) => {
    ...
    return (localMesgNum !== -1 ? localMesgNum : this.#nextLocalMesgNum++) & fit_default.LOCAL_MESG_NUM_MASK;
  };
  /**
  ...
  #writeMesgDefinitionIfNotActive = (mesgDefinition) => {
    ...
    if (this.#localMesgDefinitions[localMesgNum] == null || !this.#localMesgDefinitions[localMesgNum].equals(mesgDefinition)) {
  ...
  #nextLocalMesgNum = 0;
  #outputStream = new output_stream_default();
